...
class UpdateUserCartItemQuantityService @Inject constructor(
    private val repository: ICartRepository
) {
    suspend operator fun invoke(requestModel: CartItemRequestDto): Flow<UpdateUserCartItemQuantityResponseModel> = flow {
        emit(repository.updateUserCartItemQuantity(requestModel))
    }
}
